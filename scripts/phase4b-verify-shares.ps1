# =============================================================================
# Lab 3 - Phase 4b: Verify SMB Shares and NTFS Permissions
# Runs on: FS01
# =============================================================================
Set-ExecutionPolicy -ExecutionPolicy Bypass -Scope Process -Force
$basePath = "C:\Shares"
$pass = $true
Write-Host "=== Phase 4b: Share and NTFS Verification ===" -ForegroundColor Cyan
...
foreach ($share in $expectedACLs.Keys) {
    ...
    foreach ($expected in $expectedACLs[$share]) {
        ...
        if (-not $ace) {
            Write-Host "  [FAIL] $($expected.Identity) has no entry" -ForegroundColor Red
            $pass = $false; continue
        }
        ...
        if ($hasRight) {
            ...
            Write-Host "  [FAIL] $($expected.Identity) wrong rights" -ForegroundColor Red
            $pass = $false
        }
    }
}
...
Write-Host ""
if ($pass) { Write-Host "=== Lab 3 Verification PASSED ===" -ForegroundColor Green }
else { Write-Host "=== Lab 3 Verification FAILED ===" -ForegroundColor Red }
